...
public class LinuxStateForShell {
            ...
            if (command.equals(CPU_MEM_SHELL)) {
                ...
                for (String line : strings) {
                    ...
                    if (line.startsWith("%CPU(S):")) {
                        ...
                        try {
                            String us = line.split(":")[1].split(",")[0].replace("US", "");
                            serverInformation.setCupInfo(us.replaceAll(" ", ""));
                        } catch (Exception e) {
                            e.printStackTrace();
                            cpuStr += "计算过程出错";
                        }
                        ...
                        try {
                            ...
                            String memStr2 = memStr1.substring(0, memStr1.indexOf("TOTAL, "));
                            String memStr3 = memStr1.substring(memStr1.indexOf("TOTAL, ")+"TOTAL, ".length(), memStr1.indexOf("FREE, "));
                            String memStr4 = memStr1.substring(memStr1.lastIndexOf("FREE, ")+"FREE, ".length(), memStr1.indexOf("USED, "));
                            String memStr5 = memStr1.substring(memStr1.lastIndexOf("USED, ")+"USED, ".length(), memStr1.indexOf("BUFF/CACHE"));
                            serverInformation.setMemoryTotal(memStr2.replaceAll(" ", ""));
                            serverInformation.setMemoryFree(memStr3.replaceAll(" ", ""));
                            serverInformation.setMemoryUsed(memStr4.replaceAll(" ", ""));
                            serverInformation.setMemoryBuffers(memStr5.replaceAll(" ", ""));
                        } catch (Exception e) {
                            e.printStackTrace();
                            memStr += "计算过程出错";
                ...
                try {
                    String[] splits = commandResult.split(LINE_SEPARATOR);
                    //将硬盘信息按换行符分割
                    for (String split : splits) {
                        if (split.endsWith("/")) {
                            String[] arr = split.split("\\s+");
                            for (int i = 0; i < arr.length; i++) {
                                serverInformation.setFilesystem(arr[0]);
                                serverInformation.setSize(arr[1]);
                                serverInformation.setUsed(arr[2]);
                                serverInformation.setAvail(arr[3]);
                                serverInformation.setUserate(arr[4]);
                                serverInformation.setMountedon(arr[5]);
                            }
                        }
                    }
    ...
    public static void main(String[] args) {
        Map<String, String> result = runDistanceShell(COMMANDS, "root", "zsqcs1.6", "139.196.197.153");
